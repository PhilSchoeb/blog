...
Pour déterminer le côté de l'équipe à domicile, nous faisant un vote pour le côté le plus fréquent.
...
La courbe ROC illustre la capacité des modèles à différencier les classes positives et négatives. Random Forest a obtenu une AUC de 0,71, montrant qu'il surpasse les modèles de régression logistique. Cependant, des versions de regression logistique (distance et "both") arrive à des résultats pas si loin pour cette métrique. Ces résultats montrent que certains modèles, comme Random Forest, fonctionnent mieux que prévu en test, tandis que d'autres, comme la régression logistique basée sur l'angle, sous-performent lorsqu'on compare avec nos données de validation. 

Ensuite, nous avons un graphique montre le taux d'événements positifs (goal rate) dans différents centiles de probabilité prédite. Encore une fois, on a Random Forest qui présente un taux de réussite plus élevé dans les centiles supérieurs, ce qui est conforme à son AUC élevée. Cela montre que ses probabilités prédictives sont bien alignées avec les taux réels. Conformement à ce qu'on a observé dans la courbe ROC, les versions de regression logistique (distance et "both") présentent des performances similaires, mais légèrement inférieures. La régression basée sur l'angle, quant à lui, semble encore non-performante.

Pour le Cumulative % Goals vs Probability Centiles, les conclusions restent similaires. Le Random Forest domine clairement les autres modèles. la régression logistique basée sur distance et both offre des performances correctes mais légèrement en retrait. Finalement, le modèle basé uniquement sur l’angle montre une courbe très peu performante, confirmant une fois de plus qu’il est mal adapté à la tâche.
