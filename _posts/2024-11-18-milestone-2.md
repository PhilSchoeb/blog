...
On constate alors que le meilleur ensemble de caractéristiques est l'original. Cela démontre que le modèle XGBoost utilise toutes les caractéristiques à sa disposition. En effet, on voit une grande corrélation entre l'AUC et le nombre de caractéristiques du modèle.

De ce fait, les courbes demandées sont celles présentées dans la section 5.2.

Pour finir, voici le lien vers le projet Wandb pour cette section :

https://wandb.ai/IFT6758-2024-A05/xgboost_feature_selection?nw=nwuserphilschoeb
